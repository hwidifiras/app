...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error instanceof Error && error.message === "GROUP_CAPACITY_REACHED") {
      return NextResponse.json({ error: "Un groupe s̩lectionn̩ est complet" }, { status: 409 });
    }
    if (error instanceof Error && error.message === "GROUP_NOT_FOUND") {
      return NextResponse.json({ error: "Un groupe s̩lectionn̩ est introuvable ou inactif" }, { status: 409 });
    }
