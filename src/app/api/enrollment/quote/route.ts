    ...
    async () => {
      try {
        ...
        if (msg.includes("PLAN_SPORT_MISMATCH")) {
          return NextResponse.json(
            { error: "Le plan ne correspond pas à la discipline du cours" },
            { status: 409 },
          );
        }
        if (msg.includes("MEMBER_TYPE_MISMATCH")) {
          return NextResponse.json(
            { error: "Type d'élève incompatible avec ce cours" },
            { status: 409 },
          );
        }
        if (msg.includes("MEMBER_ARCHIVED")) {
          return NextResponse.json(
            { error: "Impossible d'inscrire un élève archivé" },
            { status: 409 },
          );
        }
        ...
        if (msg.includes("GROUP_INVALID")) {
          return NextResponse.json(
            { error: "Cours invalide ou inactif" },
            { status: 404 },
          );
        }
        console.error("[POST /api/enrollment/quote]", error);
        return NextResponse.json(
          { error: "Erreur lors du devis" },
          { status: 500 },
        );
      }
    },
