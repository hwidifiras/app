...
import { ChevronLeft, ChevronRight, QrCode, ScanLine, Waves, type LucideIcon } from "lucide-react";
...
export function AccessMediaCarousel() {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
      <div className="grid lg:grid-cols-[1.35fr_0.65fr]">
        <div className="relative aspect-[4/3] min-h-[280px] bg-slate-100 sm:aspect-[16/10] lg:aspect-auto lg:min-h-[500px]">
          ...
          <span className="absolute left-4 top-4 rounded-md border border-white/70 bg-white/95 px-3 py-2 text-xs font-bold text-slate-800 shadow-sm">
            {active.availability}
          </span>
        </div>

        <div className="flex min-h-[360px] flex-col justify-between p-6 sm:p-8 lg:min-h-[500px]">
          ...
          <div className="mt-8">
            <div className="flex flex-wrap gap-2" role="tablist" aria-label="Supports de pointage">
              {accessMedia.map((medium, index) => (
                <button
                  ...
                  className={index === activeIndex ? "rounded-md bg-blue-600 px-3 py-2 text-xs font-bold text-white" : "rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 hover:border-blue-300 hover:text-blue-700"}
